  ...
  Future<void> loadProfiles() async {
    try {
      ...
      profiles = await FirebaseDataService.loadGiftProfiles()
        ...
        onTimeout: () {
          print('⏱️ Timeout lors du chargement des profils');
          return []; // Retourner une liste vide en cas de timeout
        },
      );
      ...
      if (profiles.isNotEmpty && selectedProfileId == null) {
        ...
        try {
          await loadPersonFavorites(personId).timeout(
            const Duration(seconds: 5),
            onTimeout: () {
              print('⏱️ Timeout lors du chargement des favoris');
            },
          );
        } catch (e) {
